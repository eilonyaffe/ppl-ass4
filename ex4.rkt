...
(define pipe$
  (lambda (lst-fun cont)
    (if (empty? (cdr lst-fun))
        (cont (car lst-fun))
        (pipe$ (cdr lst-fun) (lambda (res) (compose$ (car lst-fun) res cont)))
     )
   )
)
...
(define generate-pi-approximations
  (lambda ()
    (lzl-map (lambda (x) (* 8 x)) (reduce3-lzl + 0 (lzl-map (lambda (x) (/ 1 (+ (* x x) (* 2 x)))) (integers-steps-from 1 4))))
   )
 )
